...
init(Args) -> gen_server:cast({start}), {ok,#state.options=Args}.
...
start_watch(Directory) ->
	lists:foreach(fun(A) -> inoteefy:watch(A,fun inotify_callback/1) end, subdirs(Directory)).

preprocess(Directory) -> ok.
inotify_callback({Dir_Name, Operation,Cookie, Name}) -> ok.


subdirs(Directory) -> subdirs(Directory, []).
subdirs(Directory, Accumulator) ->
	io:format("processing ~s: ", [Directory]),
